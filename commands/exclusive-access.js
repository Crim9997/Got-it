...
class VerificationManager {
    constructor() {
        ...
        this.filePath = path.join(__dirname, 'discord_verification_map.json');
    ...
    async loadFromFile() {
        try {
            ...
            console.log(`✅ Loaded ${loadedCount} active verification mappings`);
        } catch (error) {
            console.log('📁 Creating new verification mapping file');
        }
    }
    ...
    async saveToFile() {
        try {
            ...
            console.error('❌ Error saving verification map:', error);
        }
    }
    ...
    markVerified(discordUserId) {
        if (this.storage.has(discordUserId)) {
            const userData = this.storage.get(discordUserId);
            console.log(`✅ Verified: Discord ${discordUserId} -> Roblox ${userData.robloxUsername} (${userData.robloxUserId})`);
            this.storage.delete(discordUserId);
            this.saveToFile();
            return userData;
        }
    ...
    cleanExpired() {
        ...
        if (cleaned > 0) {
            console.log(`🧹 Cleaned ${cleaned} expired verification codes`);
            this.saveToFile();
        }
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Check if you own the exclusive access shirt and get your role!')
    ...
    async execute(interaction) {
        ...
        const shirtId = 144076358;
        const roleId = '1309964451025391646';
        ...
        try {
            ...
            console.log(`🔍 Checking verification for Discord: ${discordUserId} -> Roblox: ${username} (${userId})`);
            ...
            if (!ownsShirt) {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(
                        `**You don't own the required exclusive access shirt!**\n\n` +
            ...
            const successEmbed = new EmbedBuilder()
                .setColor('#ff0000')
                .setTitle('✅ Exclusive Access Granted!')
                .setDescription(
                    `**Welcome to the exclusive club, ${username}!**\n\n` +
                    `👤 **Discord:** <@${discordUserId}>\n` +
            ...
            console.error('❌ Exclusive access command error:', error);
